#!/usr/bin/env python3
"""
Malnutrition Model Evaluation Script - v1.1.0 (Multi-Step Reasoning)
====================================================================
Evaluates trained MedDialogue malnutrition models on test datasets.
Uses multi-step clinical reasoning:
  1. Gathers key evidence (anthropometrics, symptoms, intake)
  2. Analyzes diagnosis with clinical reasoning
  3. Makes final classification with JSON output

Forces JSON output: {"malnutrition_status": "Malnutrition Present/Absent"}
Prints all reasoning steps and JSON response to terminal for debugging.
...
from meddialogue.inference import InferenceEngine, ReasoningChain
logging.basicConfig(
    level=logging.INFO,
...
class MalnutritionEvaluator:
    """
    Evaluator for malnutrition classification models.
   
    Handles:
    - Model loading (adapter or merged)
    - Batch inference with progress tracking
    - JSON response enforcement
    - Terminal printing of JSON for every sample
    ...
    def __init__(
        ...
        self.tokenizer = None
        self.device = None
        self.inference_engine = None  # Will be initialized after loading model

        # Task configuration for malnutrition - now with strict JSON output
        self.task_config = TaskConfig(
            task_name="pediatric_malnutrition_evaluation",
            task_description="Evaluate malnutrition status: Present or Absent. Respond in strict JSON format.",
            input_field="txt",
            output_fields=["malnutrition_status"],
            question_templates={
                "malnutrition_status": [
                    "Is malnutrition present or absent in this patient? Answer in strict JSON: {\"malnutrition_status\": \"Malnutrition Present\"} or {\"malnutrition_status\": \"Malnutrition Absent\"}.",
                    "Classify malnutrition status. Output only valid JSON: {\"malnutrition_status\": \"Malnutrition Present\"} or {\"malnutrition_status\": \"Malnutrition Absent\"}.",
                    "Does this patient have malnutrition? Respond with exactly: {\"malnutrition_status\": \"Malnutrition Present\"} or {\"malnutrition_status\": \"Malnutrition Absent\"}."
                ]
            },
            output_formats=OutputFormat.JSON  # Enforce JSON
        )
       
        logger.info(f"Evaluator initialized for {model_type} (Multi-Step Reasoning)")
    ...
    def load_model(self, max_seq_length: int = 16384):
        ...
        # Initialize inference engine for multi-step reasoning
        ...
        logger.info("Inference engine initialized for multi-step reasoning")
        logger.info("="*80)
    ...
    def infer_single(self, clinical_note: str) -> Tuple[str, int, float]:
        ...
        Returns:
            Tuple of (raw_response, predicted_label, confidence)
        """
        # Initialize reasoning chain
        chain = ReasoningChain(self.inference_engine)
        chain.start_conversation(self.task_config.get_system_prompt())
        chain.add_context(clinical_note, "CLINICAL NOTE")
        ...
        evidence_response = chain.ask_question(evidence_question)
        ...
        reasoning_response = chain.ask_question(reasoning_question)
        ...
        final_response = chain.ask_question(final_question)
        print(f"A: {final_response}\n")

        # Try to extract JSON from final response
        json_response = self._extract_json_response(final_response)
        print("JSON OUTPUT:")
        print(json.dumps(json_response, indent=2))
        print("="*80 + "\n")

        # Parse classification from JSON (fallback to regex on all responses)
        predicted_label, confidence = self._parse_from_json_or_fallback(json_response, final_response)
    ...
    def _extract_response(self, full_output: str, prompt: str) -> str:
        ...
        if prompt in full_output:
            ...
            markers = ["assistant", "Assistant:", "<|assistant|>", "<|im_start|>assistant"]
    ...
    def evaluate_dataset(
        ...
        confidences = []
        responses = []
        json_outputs = []  # Store extracted JSON for saving
       
        logger.info("Running inference on test set...")
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating"):
            try:
                response, pred, conf = self.infer_single(row["txt"])
                # Extract JSON again for saving
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate trained malnutrition model on test set (Multi-Step Reasoning v1.1.0)",
...
This approach mirrors clinical decision-making: Gather → Reason → Classify
    ...
    logger.info("")
    logger.info("="*80)
    logger.info("MALNUTRITION MODEL EVALUATION v1.1.0 (MULTI-STEP REASONING)")
    logger.info("="*80)
